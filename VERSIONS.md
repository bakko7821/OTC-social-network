...
- В **routes/messages.ts** - добавлен роут на изменение сообщения
- В **server.ts** - для socket.io, добавлен слушатель **update_message**, который отрисовывает измененные сообщения

### Клиентская часть

- В компоненте **components/Messages/MessageItem.tsx** - реализованна логика редактирования сообщения

# commit 25.47.9

### Серверная часть

- Изменена модель **Messages.ts** - content заменен со STRING на TEXT (теперь больше 255 символов)

### Клиентска часть

- Стилизованно изменение сообщения

# commit 25.48.10

- Переделанны некоторые иконки
- Убранны незадействованные(лишние) кнопки в dropDownMenu
- Создан компонент **UserProfile.tsx**, который появляется справа от сообщений в **MainPage.tsx**
- Создан **/utils/formatPhone.ts** - функция, которая разделяет номер телефона с +00000000000 на +0 000 000 00 00

# commit 26.49.10

### Серверная часть

- В DELETE роут в **/routes/messages.ts** - добалена проверка authMiddleware, чтобы только я мог удалить своё сообщение
- В **server.ts** - измененна live отрисовка статуса пользователя
- В **server.ts** - измененна live отрисовка удаления сообщения

### Клиентская часть

- Изменена отрисовка **Chats.tsx** - теперь передается больше параметров
- В **Message.tsx** - переделанна функция удаления сообщения.
- В **MoreDropDownMenu.tsx** добавлена кнопка вызова окна профиля пользователя
...
- Переработаны типы
- Переработан **routes/messages.ts** GET api/messages/dialogs/me

### Клиентская часть

- Создана мобильная верстка

# commit LAST

- Из кода удалены все пустые файлы, папки, комментарии
- Чуть подчищена архитектура
- Создан README.md