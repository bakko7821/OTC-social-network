...
# commit 18.33.8 (Пофикшено отображение собеседника)

### Серверная часть

- Изменен роут **routes/messages.ts**
- Скопирован файл **/server/src/types.ts**
- Добавлена логика изменения статуса online
...
- Добавлен поиск по username

### Клиентская часть

- Добавлена возможность найти пользователя по username и написать ему
- Добавленно окно подтвержденя при удалении чата

# commit 21.38.8

### Серверная часть

- Добавлен роут на удаление сообщения по ID

### Клиентская чать

- В MessageItem добавлен event.listener, на ПКМ по сообщению 
- Добавленна операция удаления одного сообщения.

# commit 21.39.8

- Все стили для Message.tsx перенесены в message.scss
- Добавленна возможность копирования сообщения в буфер обмена

# commit 21.40.8

- Исправлен баг с темой, теперь тема обновляется при загрузке страницы.
...
- В **routes/users.ts** - создан PUT роут на изменение данных

### Клиентская часть

- Закончен элемент **EditProfileAlert.tsx**
- Создан файл **_keyframes.scss** - для анимаций
- К каждой кнопке добавленна интересная анимация
...
- Создан **middleware/multer.ts**
- Создан **routes/upload.ts**

### Клиентская часть

- Добавленна возможность загрузки аватара пользователя